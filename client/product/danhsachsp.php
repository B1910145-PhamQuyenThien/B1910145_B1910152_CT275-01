<?php
    ...
    INNER JOIN brands b ON p.brand_id = b.brand_id ORDER BY prd_id DESC LIMIT ".$item_per_page." OFFSET ".$offset."");
    $totalRecords = $connect -> query("SELECT * FROM products");
        ...
        <div class="row mb-4">
            ...
            <div class="col-10 bg-light">
                <div class="row mb-4 bg-light text-light shadow pl-3 pt-3 pr-3">
                    ...
                    <div class="col">
                        <form class="text-right" role="search" action="index_client.php?page_layout=sapxep"
                            method="POST" enctype="multipart/form-data">
                            <select class="form-select form-select-sm w-50" style="display: inline"
                                aria-label=".form-select-sm example" name="sort">
                                <option value="1" selected>Giá tăng dần</option>
                                <option value="2">Giá giảm dần</option>
                                <!-- <option value="3">Mới tới cũ</option>
                                <option value="4">Cũ tới mới</option> -->
                            </select>
                            <button type="submit" name="sbm" class="btn btn-primary btn-sm">Lọc</button>
                        </form>
                    </div>
                </div>
                        ...
                        while($row = $result -> fetch()){ 
                            ...
                            "<div class='col-md-3 shadow-sm mb-2'>
                                <a class='mb-2' href='index_client.php?page_layout=chitietsp&id=".$row['prd_id']."'>
                                    ...
                                    <h5 class='text-secondary text-center'>".number_format($row['price'],0,",",".")." VNĐ</h5>
